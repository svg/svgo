...
async function action(args, opts, command) {
  ...
  if (opts.precision != null) {
    precision = Number.parseInt(opts.precision, 10);
  ...
  if (
    ...
    if (!match) {
      throw Error(`Invalid Node.js version ${PKG.engines.node}`);
    }
    var nodeVersion = match[0];
 ...
 *
 * @param {CommandConfig} config options
...
function checkOptimizeFileError(config, input, output, error) {
  if ('code' in error) {
    ...
    } else if (error.code == 'ENOENT' && 'path' in error) {
...
function checkWriteFileError(input, output, data, error) {
  if ('code' in error && error.code == 'EISDIR' && input) {
...
function showAvailablePlugins() {
  const list = builtin
    ...
    .map((plugin) => {
      let out = ` [ ${colors.green(plugin.name)} ]`;
