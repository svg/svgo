...
module.exports = require('coa').Cmd()
    ...
    .opt()
        ...
        .end()
    .opt()
        .name('in-place').title('Optimize many filenames at once, each listed on the CLI')
    ...
    .arg()
        ...
        .end()
    .arg()
        .name('rest').title('Rest of the files to optimize (used with --in-place)')
        .arr()
        .end()
    .act(function(opts, args) {
        ...
        // Has [rest [...rest]] without --in-place
        if (args.rest && !opts['in-place']) {
            console.error(`Error: unexpected arguments: ${args.rest.join(' ')} -- did you mean to use --in-place?`);
            process.exit(1);
        }
        ...
        function processFilename(filename, outputFilename) {
            ...
            return new Promise((resolve, reject) => {
                FS.readFile(input, 'utf8', function(err, data) {
                    ...
                    optimizeFromString(data, config, opts.datauri, input, outputFilename)
                        .then(resolve)
                        .catch(reject);
                });
            });
        ...
        if (opts['in-place']) {
            let filenames = [];
            if (args.input) {
                filenames.push(args.input);
            }
            if (args.output) {
                filenames.push(args.output);
            }
            filenames.push.apply(filenames, args.rest);
            return Promise.all(filenames.map(function (filename) {
                return processFilename(filename, filename);
            }));
        // --input
        } else if (input) {
            // STDIN
