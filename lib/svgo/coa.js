...
module.exports = require('coa').Cmd()
    ...
    .opt()
        ...
        .end()
    .opt()
        .name('apply-plugins-at-once').title('Apply each plugin in separate traverse of the AST tree')
        .long('apply-plugins-at-once')
        .flag()
        .end()
    .opt()
        .name('report-profit-after-every-pass').title('Report non-zero profit information after each traverse of AST tree')
        .long('report-profit-after-every-pass')
        .flag()
        .end()
    .arg()
    ...
    .act(function(opts, args) {
        ...
        // --apply-plugins-at-once
        if (opts['apply-plugins-at-once']) {
            config.applyPluginsAtOnce = true;
        }

        // --report-profit-after-every-pass
        if (opts['report-profit-after-every-pass']) {
            config.reportProfitAfterEveryPass = true;
        }
...
function processSVGData(config, info, data, output, input) {
    ...
    var pluginApplyStrategyConfig = {
        reportCallback: reportProfit,
        inBytes: prevFileSize,
        js2svg: config.js2svg,
        applyPluginsAtOnce: config.applyPluginsAtOnce,
        reportProfitAfterEveryPass: config.reportProfitAfterEveryPass,
    };
    return svgo.optimize(data, info, pluginApplyStrategyConfig).then(function (result) {
        if (config.datauri) {
...
function reportProfit(multipassIteration, plugins, inBytes, currentBytes, outBytes) {
    printDeltaProfitInfo(chalk.yellow('Iter. ' + multipassIteration + ': '), inBytes, currentBytes - outBytes, outBytes);
    console.log('    ' + plugins.map(x => chalk.gray(x.name)).join(", "));
}
...
function printProfitInfo(inBytes, outBytes) {
    printDeltaProfitInfo('', inBytes, inBytes - outBytes, outBytes);
}

function printDeltaProfitInfo(context, inBytes, delta, outBytes) {
    var profitPercents = delta / inBytes * 100;
    var coloring = profitPercents < 0 ? chalk.red : chalk.green;
    console.log(
        context +
        (Math.round((inBytes / 1024) * 1000) / 1000) + ' KiB' +
        (profitPercents < 0 ? ' + ' : ' - ') +
        coloring(Math.abs((Math.round(profitPercents * 10) / 10)) + '%') + ' = ' +
