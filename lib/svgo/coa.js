...
function printProfitInfo(inBytes, outBytes) {
    let profitPercents = 100 - outBytes * 100 / inBytes;
    let profitPercentsOutput = Math.abs((Math.round(profitPercents * 10) / 10));
    profitPercentsOutput = chalk.keyword((profitPercents < 0) ? 'red':'green')(profitPercentsOutput + '%')

    console.log(
        (Math.round((inBytes / 1024) * 1000) / 1000) + ' KiB' +
        (profitPercents < 0 ? ' + ' : ' - ') +
        profitPercentsOutput + ' = ' +
