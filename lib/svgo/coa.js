...
function processSVGData(config, info, data, output, input) {
    ...
    return svgo.optimize(data, info, pluginApplyStrategy(prevFileSize, config)).then(function (result) {
        if (config.datauri) {
...
function pluginApplyStrategy(inBytes, config) {
    let lastReportBytes = inBytes;
    let multipassIteration = 1;
    const shouldReportProfit = config.reportProfitAfterEveryPass;
    function applyCallbackAndReportProfit(data, plugins, callback) {
        callback(plugins);
        if (!shouldReportProfit) {
            return;
        }
        const currentBytes = JS2SVG(data, config.js2svg).data.length;
        if (currentBytes === lastReportBytes) {
            return;
        }
        const profitPercents = (lastReportBytes - currentBytes) / inBytes * 100;
        console.log(
            chalk.yellow('Iter. ' + multipassIteration + ': ') +
            (Math.round((inBytes / 1024) * 1000) / 1000) + ' KiB' +
            (profitPercents < 0 ? ' + ' : ' - ') +
            chalk.green(Math.abs((Math.round(profitPercents * 10) / 10)) + '%') + ' = ' +
            (Math.round((currentBytes / 1024) * 1000) / 1000) + ' KiB'
        );
