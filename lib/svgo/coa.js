...
function processDirectory(config, dir, files, output) {
    if(config.recursive) {
        var folders = files.filter(name => checkIsDir(PATH.resolve(dir, name)));

        Promise.all(folders.map(name => {
            const recursiveOut = PATH.resolve(output, name);
            if(!FS.existsSync(recursiveOut)) {
                FS.mkdirSync(recursiveOut);
            }

            optimizeFolder(config, PATH.resolve(dir, name), recursiveOut);
        }));
    }
