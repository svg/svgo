...
CSSStyleDeclaration.prototype.getCssText = function() {
  var declarationList = csstree.parse('', {context: 'declarationList'});
  this.properties.forEach(function(property, propertyName) {
    var strImportant   = (property.priority === 'important' ? '!important' : '');
    var declarationStr = propertyName + ':' + property.value + strImportant;
    var declaration    = csstree.parse(declarationStr, {context: 'declaration'});

    declarationList.children.appendData(declaration);
  });

  return csstree.translate(declarationList);
};
