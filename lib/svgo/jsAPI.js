 ...
 * Also Removes a particulate value of an attribute 
 * Example : Removal class A where attribute is class="A B" 
 ...
 */
 JSAPI.prototype.removeAttr = function(name, val, recursive) {
    ...
    if( [' ',val].every(i => this.attrs[name].value.includes(i)) ){
        var newValue = this.attrs[name].value.split(' ').filter(values => values != val ).join(' ');
        this.attrs[name].value = newValue;
        return true;
    }
