 ...
 */
 JSAPI.prototype.hasAttrLocal = function(localName, val) {
     return this.attrLocal(localName, val) !== undefined;
};
...
/**
 * Get a specific attribute from an element
 * (by local name or local name + value).
 *
 * @param {String} name attribute name
 * @param {String} [val] attribute value (will be toString()'ed)
 * @return {Object|Undefined}
 */
 JSAPI.prototype.attrLocal = function(localName, val) {

    if (!this.attrs || !Object.keys(this.attrs).length) return undefined;
...
/**
 * Returns the first attribute that passes the test, if any.
 *
 * @param {Function} callback test to apply to each attribute
 * @param {Object} [context] test parameters
 * @return {Object|undefined}
 */
 JSAPI.prototype.someAttr = function(callback, context) {

    if (!this.hasAttr()) return undefined;

    for (var name in this.attrs) {
        var attr = this.attrs[name];
        if (callback.call(context, attr)) return attr;
    }
