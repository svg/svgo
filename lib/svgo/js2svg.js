 ...
 * @return {String}
 */
JS2SVG.prototype.convert = function (node) {
  var svg = '';
  ...
  for (const child of node.children) {
    if (child.type === 'element') {
      svg += this.createElem(child);
    }
    if (child.type === 'text') {
      svg += this.createText(child, node);
    }
    if (child.type === 'doctype') {
      svg += this.createDoctype(child);
    }
    if (child.type === 'instruction') {
      svg += this.createProcInst(child);
    }
    if (child.type === 'comment') {
      svg += this.createComment(child);
    }
    if (child.type === 'cdata') {
      svg += this.createCDATA(child);
    }
  }
 ...
 * @return {String}
 */
JS2SVG.prototype.createText = function (node, parentNode) {
  let { value } = node;
  if (parentNode.name !== 'style' && parentNode.name !== 'script') {
    value = value.replace(this.config.regEntities, this.config.encodeEntity);
  }
  return (
    this.createIndent() +
