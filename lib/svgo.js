...
SVGO.prototype.optimize = function(svgstr, info, pluginApplyStrategy) {
    info = info || {};
    return new Promise((resolve, reject) => {
        ...
        var config = this.config,
            ...
            optimizeOnceCallback = (svgjs) => {
                ...
                if (++info.multipassCount < maxPassCount && svgjs.data.length < prevResultSize) {
                    ...
                    this._optimizeOnce(svgjs.data, info, optimizeOnceCallback, pluginApplyStrategy);
                } else {
                    if (config.datauri) {
        ...
        this._optimizeOnce(svgstr, info, optimizeOnceCallback, pluginApplyStrategy);
    });
};
