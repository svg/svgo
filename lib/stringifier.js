 ...
 * }}
 */
const stringifySvg = (data, { indent, ...userOptions } = {}) => {
  let newIndent = '';
  if (typeof indent == 'number' && Number.isNaN(indent) === false) {
    newIndent = indent < 0 ? '\t' : ' '.repeat(indent);
  } else if (typeof indent !== 'string') {
    newIndent = defaults.indent;
  }
  /**
   * @type {Options}
   */
  const config = { ...defaults, ...userOptions, indent: newIndent };
  /**
   * @type {State}
   */
  const state = {
...
const createIndent = (config, state) => {
  ...
  if (config.pretty && state.textContext == null) {
    indent = config.indent.repeat(state.indentLevel - 1);
  }
  return indent;
};
