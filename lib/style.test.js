...
describe('includesAttrSelector', () => {
  it('matches attribute existence selector [attr]', () => {
    expect(
      includesAttrSelector('[preserveAspectRatio]', 'preserveAspectRatio'),
    ).toBe(true);
    expect(includesAttrSelector('[preserveAspectRatio]', 'fill')).toBe(false);
  });

  it('matches with attrValue and exists action', () => {
    expect(
      includesAttrSelector(
        '[preserveAspectRatio]',
        'preserveAspectRatio',
  ...
  it('matches equals selector [attr=val]', () => {
    expect(includesAttrSelector('[fill=red]', 'fill', null, false, 'red')).toBe(
      true,
    );
    expect(
      includesAttrSelector('[fill=red]', 'fill', null, false, 'blue'),
    ).toBe(false);
  });

  it('matches starts-with selector [attr^=val]', () => {
    expect(
      includesAttrSelector(
  ...
  it('matches ends-with selector [attr$=val]', () => {
    expect(
      includesAttrSelector(
        '[id$=-important]',
        'id',
    ...
    expect(
      includesAttrSelector('[id$=-important]', 'id', null, false, 'box-normal'),
    ).toBe(false);
  });

  it('matches contains selector [attr*=val]', () => {
    expect(
      includesAttrSelector(
        '[class*=btn]',
        'class',
  ...
  it('matches whitespace-separated word selector [attr~=val]', () => {
    expect(
      includesAttrSelector(
        '[class~=btn]',
        'class',
    ...
    expect(
      includesAttrSelector(
        ...
        null,
        false,
        'inactive btn2 primary',
      ),
    ).toBe(false);
  });

  it('matches hyphen selector [attr|=val]', () => {
    expect(includesAttrSelector('[lang|=en]', 'lang', null, false, 'en')).toBe(
      true,
    );
    expect(
      includesAttrSelector('[lang|=en]', 'lang', null, false, 'en-US'),
    ).toBe(true);
    expect(includesAttrSelector('[lang|=en]', 'lang', null, false, 'fr')).toBe(
      false,
    );
  });

  it('returns true (conservative) when attrValue is not provided', () => {
    expect(includesAttrSelector('[fill^=r]', 'fill')).toBe(true);
    expect(includesAttrSelector('[class*=btn]', 'class')).toBe(true);
  });
  ...
  it('handles empty selector values in ^=, $=, *= (no match per CSS spec)', () => {
    expect(includesAttrSelector('[fill^=""]', 'fill', null, false, 'red')).toBe(
      false,
    );
    expect(includesAttrSelector('[fill$=""]', 'fill', null, false, 'red')).toBe(
      false,
  ...
  it('still supports the value parameter for exact selector value matching', () => {
    // .btn generates {name: "class", action: "element", value: "btn"}
    expect(includesAttrSelector('.btn', 'class', 'btn')).toBe(true);
    expect(includesAttrSelector('.btn', 'class', 'other')).toBe(false);
    // #main generates {name: "id", action: "equals", value: "main"}
    expect(includesAttrSelector('#main', 'id', 'main')).toBe(true);
    expect(includesAttrSelector('#main', 'id', 'other')).toBe(false);
  });

  it('issue example: [preserveAspectRatio^=y] does not match xMidYMid meet', () => {
    expect(
      includesAttrSelector(
        '[preserveAspectRatio^="y"]',
        'preserveAspectRatio',
