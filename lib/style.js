...
/**
 * Checks if a CSS attribute selector action matches a given element value.
 *
 * @param {string} action - The css-what attribute action type.
 * @param {string} selectorValue - The value in the CSS selector.
 * @param {string} attrValue - The actual attribute value on the element.
 * @returns {boolean}
 */
const matchesAttrAction = (action, selectorValue, attrValue) => {
  switch (action) {
    case 'exists':
      return true;
    case 'equals':
      return attrValue === selectorValue;
    case 'start':
      return selectorValue.length > 0 && attrValue.startsWith(selectorValue);
    case 'end':
      return selectorValue.length > 0 && attrValue.endsWith(selectorValue);
    case 'any':
      return selectorValue.length > 0 && attrValue.includes(selectorValue);
    case 'element':
      return attrValue.split(/\s+/).includes(selectorValue);
    case 'hyphen':
      return (
        attrValue === selectorValue || attrValue.startsWith(selectorValue + '-')
      );
    default:
      // Unknown action, assume it matches to be safe.
      return true;
  }
};
 ...
 * respectively.
 *
 * When `attrValue` is provided, the selector's operator (e.g. `^=`, `*=`) is
 * evaluated against the element's actual attribute value to determine if the
 * selector would match. This avoids conservatively keeping attributes that
 * would not actually be affected by the CSS rule.
 *
...
export const includesAttrSelector = (
  ...
  for (const subselector of selectors) {
    const hasAttrSelector = subselector.some((segment, index) => {
      ...
      if (value != null && segment.value !== value) {
        return false;
      }

      if (attrValue != null) {
        return matchesAttrAction(segment.action, segment.value, attrValue);
      }
