...
import { parseSvg, SvgoParserError } from './parser.js';
...
test('warn when user tries enable plugins in preset', () => {
  ...
  expect(warn)
    .toHaveBeenCalledWith(`You are trying to configure cleanupListOfValues which is not part of preset-default.
...
test('optimize returns the ast', () => {
  ...
  const { data, ast } = optimize(svg, config);
  expect(data).toMatchInlineSnapshot(stringifySvg(ast, config.js2svg));
  expect(ast).toMatchObject(parseSvg(data));
});

test('optimize returns the latest ast when multipass', () => {
  const svg = `
    <?xml version="1.0" encoding="utf-8"?>
    <svg viewBox="0 0 120 120">
      <desc>
