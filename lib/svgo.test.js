...
test('processes SVG with embedded styles using css-tree', () => {
  const svg = `
    <svg viewBox="0 0 100 100">
      <style>
        .red { fill: #ff0000; }
        .blue { fill: #0000ff; stroke: #000; stroke-width: 2px; }
        @media screen {
          .responsive { opacity: 0.5; }
        }
      </style>
      <rect class="red" x="10" y="10" width="30" height="30"/>
  ...
  expect(data).toContain('fill');
  expect(data).toContain('stroke');
  expect(data).toContain('<circle');
});

test('inlineStyles plugin processes CSS correctly', () => {
  const svg = `
    <svg viewBox="0 0 100 100">
      <style>
        .test { fill: green; }
      </style>
      <rect class="test" x="0" y="0" width="100" height="100"/>
    </svg>
  `;
  const { data } = optimize(svg, {
  ...
  expect(data).toContain('fill');
  expect(data).toContain('green');
});

test('prefixIds plugin processes CSS selectors correctly', () => {
  const svg = `
    <svg viewBox="0 0 100 100">
      <style>
        #myId { fill: red; }
      ...
      <rect id="myId" class="myClass" x="0" y="0" width="100" height="100"/>
    </svg>
  `;
  const { data } = optimize(svg, {
    plugins: [{ name: 'prefixIds', params: { prefix: 'svg1' } }],
  });
  expect(data).toContain('svg1');
  expect(data).toContain('#svg1');
});
