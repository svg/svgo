...
exports.fn = function (root, validateResult, params) {
  if (params || !utils.isEmpty(params)) {
    ...
    const darkmodeMaskElements = utils.findAllElementByAttributeValue(
      svg,
      'id',
      'darkmodeMask'
    );
    ...
    if (darkmodeMaskElements.length > 0) {
      const darkmodeMaskElement = darkmodeMaskElements[0];

      const [darkmodeFillWidth, darkmodeFillHeight] = params.darkmodeFillSize;

      for (const color of params.fillColors) {
        if (
          utils.findAllElementByAttributeValue(
            darkmodeMaskElement,
            'fill',
            color
          ).length > 0 ||
